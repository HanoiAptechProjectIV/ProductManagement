...
<c:forEach begin="1" end="${pageQuantity}" var="i">
    <c:choose>
        ...
        <c:when test="${page == (i+1)}">
            ...
            <a id="previous"href="<%=servletPath%>?page=${i}"
               style="display: inline-block;">Previous</a>
        </c:when>

        <c:when test="${page == (i-1)}">
            <a id="next" href="<%=servletPath%>?page=${i}"
...
|    
<form action="${pageContext.request.contextPath}/<%=servletPath%>"
