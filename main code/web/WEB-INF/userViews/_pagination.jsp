...
<%
    ...
    String userIdStr = "";
    String parameterInForm = "";
    if(servletPath.contains("history")){
        User loginedUser = (User)session.getAttribute("loginedUser");
        userIdStr = "userId="+loginedUser.getId()+"&";
        parameterInForm = "?"+userIdStr.substring(0, userIdStr.length()-1);
    }
%>
...
<c:forEach begin="1" end="${pageQuantity}" var="i">
    <c:choose>
        <c:when test="${i == 1 && page != i}">
            <li><a href="${pageContext.request.contextPath}<%=servletPath%>?<%=userIdStr%>page=1&sortBy=${sortByObj}"
                   style="display: inline-block">First</a></li>
        </c:when> 

        <c:when test="${i == pageQuantity && page != i}">
            <li><a href="${pageContext.request.contextPath}<%=servletPath%>?<%=userIdStr%>page=${pageQuantity}&sortBy=${sortByObj}"
               style="display: inline-block">Last</a></li>
        </c:when> 

        <c:when test="${page < (i-1) || page > (i+1)}">
            <li><a href="${pageContext.request.contextPath}<%=servletPath%>?<%=userIdStr%>page=${i}&sortBy=${sortByObj}"
        ...
        <c:when test="${page == (i+1)}">
            ...
            <li><a id="previous"href="${pageContext.request.contextPath}<%=servletPath%>?<%=userIdStr%>page=${i}&sortBy=${sortByObj}"
               style="display: inline-block;">Prev</a></li>
        </c:when>

        <c:when test="${page == (i-1)}">
            <li><a id="next" href="${pageContext.request.contextPath}<%=servletPath%>?<%=userIdStr%>page=${i}&sortBy=${sortByObj}"
        ...
        <c:otherwise>
            <li><a class="active" href="${pageContext.request.contextPath}<%=servletPath%>?<%=userIdStr%>page=${i}&sortBy=${sortByObj}"
...
<c:if test="<%=pageObj != null%>">
    <form action="${pageContext.request.contextPath}<%=servletPath%><%=parameterInForm%>"
